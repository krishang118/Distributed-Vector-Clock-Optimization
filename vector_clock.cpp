...
private:
    ...
    vector<int> clock;
public:
    VectorClock(int num_processes, int rank) : n(num_processes), my_rank(rank) {
        clock.resize(n, 0);}
    void increment() {
        clock[my_rank]++;}
    vector<int> get_clock() {
        return clock;}
    void update(const vector<int>& received_clock) {
        for (int i = 0; i < n; i++) {
...
int main(int argc, char** argv) {
    ...
    while (messages_sent < m) {
        ...
        if (flag) {
            ...
            vector<int> received_vc(received_data.begin() + 1, received_data.end());
        ...
        if (messages_sent < m) {
            ...
            if (prob < send_prob && !adjacency_list[world_rank].empty()) {
                ...
                vector<int> current_vc = vc.get_clock();
                send_data.insert(send_data.end(), current_vc.begin(), current_vc.end());
                MPI_Send(send_data.data(), send_data.size(), MPI_INT, dest, 0, MPI_COMM_WORLD);
                total_vc_entries_sent += n;
                messages_sent++;
            } else {
                vc.increment();
                Event e;
    ...
    if (world_rank == 0) {
        ...
        ofstream stats("vector_clock_stats.txt");
    ...
    for (int i = 0; i < n; i++) {
        if (world_rank == i) {
            ofstream logfile;
            if (i == 0) {
                logfile.open("vector_clock_log.txt", ios::out);
            } else {
                logfile.open("vector_clock_log.txt", ios::app);}
